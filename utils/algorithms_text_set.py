...
def splice_data_approx(data, text_features, texts, layer, head, seed, iters, rank, device):
    """
    ...
    tag_prefix = f"Layer_{layer}/Head_{head}/Seed_{seed}"
    ...
    if layer == -1:
        writer.add_scalar(f"Rank", rank, 13*100 + 12*10)
        writer.add_histogram("Strength_mean_val", mean_values_att, 13*100 + 12*10)
        writer.add_histogram("Strength_mean_val_text", torch.from_numpy(mean_values_text).to(device) @ vh.T @ vh, 13*100 + 12*10)
    else:
        writer.add_scalar(f"Rank", rank, layer*100 + head*10)
        writer.add_histogram("Strength_mean_val", mean_values_att, layer*100 + head*10)
        writer.add_histogram("Strength_mean_val_text", torch.from_numpy(mean_values_text).to(device) @ vh.T @ vh, layer*100 + head*10)
    ...
    patience = 100  # Number of epochs to wait for improvementy
    # Initialize variables for early stopping
    prev_cum_sum = None
    prev_relative_strength = None
    stabilization_window = 800  # Number of iterations to check stability
    cum_sum_stable_count = 0  # Counter for cumulative sum stability
    relative_strength_stable_count = 0  # Counter for relative strength stability
    stabilization_threshold = 1  # Percentage
    early_stop = False
    ...
    for epoch in range(epochs_main):
        ...
        if epoch % patience == 0:
            ...
            writer.add_scalar(f"{tag_prefix}/abs_strength", tot_str, epoch)

            writer.add_scalar(f"{tag_prefix}/indexes_cum_sum", cum_sum, epoch)

            writer.add_histogram(f"{tag_prefix}/weights_A", A, epoch)
            writer.add_histogram(f"{tag_prefix}/weights_A_top", A[:, indexes], epoch)
            writer.add_histogram(f"{tag_prefix}/weights_A_top", A[:, indexes], epoch)

            writer.add_histogram(f"{tag_prefix}/indexes", indexes, epoch)

            writer.add_histogram(f"{tag_prefix}/top_std_features_abs", 100*text_features_std[indexes][:iters]/all_str, epoch)
        ...
        if prev_cum_sum is not None:
            cum_sum_change = abs((cum_sum - prev_cum_sum) / prev_cum_sum * 100)
            if cum_sum_change < stabilization_threshold:
        ...
        if prev_relative_strength is not None:
            ...
            if relative_strength_change < stabilization_threshold:
